### Centrality of US Psychologists & Philosophers ca. 1900
### Christopher D. Green, York University, christo@yorku.ca

### Modified from K. J. Healy's "revere.R"
### "Using Metedata to find Paul Revere"
...
data.csv <- read.csv("data/memberships.csv", row.names = 1)
data <- as.matrix(data.csv)
...
pdf(file = "figures/psychphil-network.pdf", width = 22, height = 17,pointsize = 8)
...
png(file = "figures/psychphil-network.png", width = 2200, height = 1700, res = 150)
la <- layout.fruchterman.reingold(person.g)
e.wt <- get.edge.attribute(person.g, "weight")
plot(person.g, layout = la, vertex.size = 3,edge.width = 0.1,
     vertex.label = V(person.g)$name)
...
pdf(file = "figures/psychphil-network-reduced.pdf", width = 22, height = 17,pointsize = 8)
...
png(file = "figures/psychphil-network-reduced.png", width = 2200, height = 1700, res = 140)
person.g.copy <- person.g
la <- layout.fruchterman.reingold(person.g.copy)
plot(person.g.copy, layout = la, vertex.size = 3,
     vertex.label = V(person.g.copy)$name)
...
png(file = "figures/psychphil-test.png", width = 2200, height = 1700)
...
png(file = "figures/psychphil-test.png", width = 2000, height = 1000)
